...
public class TercerActivityProfe extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tercer_profe);




        String usuario_registrado1 = getIntent().getExtras().getString("user_name");
        String email_register_obtenido1 = getIntent().getExtras().getString("email_register");

        SharedPreferences.Editor editor = getSharedPreferences("usuario_recibido", MODE_PRIVATE).edit();
        editor.putString("usuario_register", usuario_registrado1);
        editor.apply();

        SharedPreferences.Editor editor1 = getSharedPreferences("email_recibido", MODE_PRIVATE).edit();
        editor1.putString("email_register", email_register_obtenido1);
        editor1.apply();


        textUser = (TextView) findViewById(R.id.editTextUserProfe);

        textUser.setText(usuario_registrado1);

        pass = (EditText)findViewById(R.id.editTextPasswordProfe);
        UserPerfil = (EditText)findViewById(R.id.editTextUserProfe);
        imageView = findViewById(R.id.imageViewProfe);
        ...
        register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                db.collection("fotoPerfil").whereEqualTo("usuario",email_register_obtenido1).get()
                            ...
                            public void onComplete(@NonNull @NotNull Task<QuerySnapshot> task) {
                                if(task.isSuccessful()){
                                    for (QueryDocumentSnapshot documentSnapshot : task.getResult()){
                                        ...
                                        db2.collection("users").document(email_register_obtenido1).update(dateUpdate).addOnSuccessListener(new OnSuccessListener<Void>() {
                                            @Override
